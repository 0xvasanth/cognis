...
//! cognis2_graph::GraphState trait locally and a __merge_json helper so the
...
mod cognis2_graph {
    ...
    pub fn __merge_json(target: &mut serde_json::Value, source: serde_json::Value) {
        // Trivial stub: overwrite. The real impl in cognis2-graph deep-merges.
        *target = source;
    }
}

#[derive(Default, Clone, Debug, Serialize, Deserialize, GraphStateV2)]
#[graph_state(crate_path = "crate::cognis2_graph")]
...
fn apply_merge_replaces_via_helper() {
    ...
    // Stub helper overwrites — real cognis2_graph deep-merges in Plan #2.
