...
use cognis_core::runnables::schema::{EventData, StreamEvent};
...
async fn test_sequence_recursion_limit() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        CognisError::RecursionLimitExceeded(_)
    ));
}
...
async fn test_fallbacks_all_fail() {
    ...
    let fallback = Arc::new(RunnableLambda::new("fallback", |_v: Value| async move {
        Err(CognisError::Other("fallback also failed".into()))
    })) as Arc<dyn Runnable>;
...
async fn test_router_missing_key() {
    ...
    let result = router
        ...
        .await;
    assert!(matches!(result.unwrap_err(), CognisError::InvalidKey(_)));
}
...
use cognis_core::runnables::RunnableExt;
...
async fn test_configurable_default() {
    ...
    let configurable = RunnableConfigurableFields::new(
        ...
        vec![ConfigurableField::new(
            "model",
            "Model",
            cognis_core::runnables::ConfigurableFieldType::String,
        )],
    );
...
async fn test_configurable_with_alternative() {
    ...
    let mut config = cognis_core::runnables::RunnableConfig::default();
...
async fn test_ext_with_fallbacks_primary_fails() {
    let primary = RunnableLambda::new("primary", |_v: Value| async move {
        Err(CognisError::Other("fail".into()))
    });
    let fallback = Arc::new(RunnableLambda::new("fallback", |v: Value| async move {
        Ok(json!({"recovered": v}))
    })) as Arc<dyn Runnable>;
...
async fn test_ext_with_retry_recovers() {
    ...
    let lambda = RunnableLambda::new("flaky", move |v| {
        ...
        async move {
            let count = c.fetch_add(1, Ordering::SeqCst);
            if count < 2 {
                Err(CognisError::Other("transient".into()))
            } else {
                Ok(v)
            }
        }
...
async fn test_ext_with_retry_exhausted() {
    let lambda = RunnableLambda::new("fail", |_v: Value| async move {
        Err::<Value, _>(CognisError::Other("always fails".into()))
    });
...
impl Runnable for MultiChunkRunnable {
    ...
    ) -> cognis_core::error::Result<Value> {
    ...
    async fn stream(
        ...
        _input: Value,
        _config: Option<&RunnableConfig>,
    ) -> cognis_core::error::Result<cognis_core::runnables::RunnableStream> {
        let chunks: Vec<cognis_core::error::Result<Value>> =
...
async fn test_ext_pipe_with_fallbacks() {
    let fail_step = RunnableLambda::new("fail", |_v: Value| async move {
        Err(CognisError::Other("step failed".into()))
    });
