...
///
/// Implementations inspect a [`RustChainError`] and return `true` if the
...
///
/// ```rust
/// use rustchain_core::retry::{RetryClassifier, DefaultRetryClassifier};
/// use rustchain_core::error::RustChainError;
///
/// let classifier = DefaultRetryClassifier;
///
/// let rate_limit = RustChainError::HttpError { status: 429, body: "Too Many Requests".into() };
/// assert!(classifier.is_retryable(&rate_limit));
///
/// let not_found = RustChainError::HttpError { status: 404, body: "Not Found".into() };
...
impl RetryClassifier for AlwaysRetryClassifier {
    fn is_retryable(&self, _error: &RustChainError) -> bool {
        true
    }
}
...
mod tests {
    ...
    fn test_retry_classifier_rate_limit() {
        let classifier = DefaultRetryClassifier;
        let err = RustChainError::HttpError {
            status: 429,
    ...
    fn test_retry_classifier_other_with_keywords() {
        ...
        let rate = RustChainError::Other("rate limit exceeded".into());
        assert!(classifier.is_retryable(&rate));

        let timeout = RustChainError::Other("request timeout after 30s".into());
        assert!(classifier.is_retryable(&timeout));

        let conn = RustChainError::Other("connection reset by peer".into());
    ...
    fn test_retry_classifier_non_retryable_errors() {
        ...
        let parse_err = RustChainError::OutputParserError {
        ...
        let tool_err = RustChainError::ToolException("tool failed".into());
        assert!(!classifier.is_retryable(&tool_err));

        let generic = RustChainError::Other("some unknown error".into());
    ...
    fn test_always_retry_classifier() {
        ...
        let err = RustChainError::Other("anything".into());
        assert!(classifier.is_retryable(&err));
    }
