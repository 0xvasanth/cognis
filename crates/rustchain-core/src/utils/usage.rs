...
pub fn dict_int_add(a: &HashMap<String, Value>, b: &HashMap<String, Value>) -> HashMap<String, Value> {
    ...
    for (key, b_val) in b {
        let merged = match (result.get(key), b_val) {
            ...
            (Some(Value::Object(m1)), Value::Object(m2)) => {
                let h1: HashMap<String, Value> = m1.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
                let h2: HashMap<String, Value> = m2.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
