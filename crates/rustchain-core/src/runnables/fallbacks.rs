...
/// Returns the variant name of a `RustChainError` for matching against
/// the `exceptions_to_handle` filter.
fn error_variant_name(err: &RustChainError) -> &'static str {
    match err {
        RustChainError::OutputParserError { .. } => "OutputParserError",
        RustChainError::ContextOverflow(_) => "ContextOverflow",
        RustChainError::TracerError(_) => "TracerError",
        RustChainError::InvalidKey(_) => "InvalidKey",
        RustChainError::SerializationError(_) => "SerializationError",
        RustChainError::NotImplemented(_) => "NotImplemented",
        RustChainError::ToolException(_) => "ToolException",
        RustChainError::ToolValidationError(_) => "ToolValidationError",
        RustChainError::SchemaAnnotationError(_) => "SchemaAnnotationError",
        RustChainError::RecursionLimitExceeded(_) => "RecursionLimitExceeded",
        RustChainError::TypeMismatch { .. } => "TypeMismatch",
        RustChainError::HttpError { .. } => "HttpError",
        RustChainError::IoError(_) => "IoError",
        RustChainError::Other(_) => "Other",
    }
}

/// Returns `true` if the error should trigger fallback behaviour based on the
/// configured exception filter.
fn should_fallback(err: &RustChainError, filter: &Option<Vec<String>>) -> bool {
    match filter {
        None => true,
        Some(allowed) => {
...
mod tests {
    ...
    impl Fails {
        fn other(label: &'static str, msg: &str) -> Self {
            Self {
                label,
                error: RustChainError::Other(msg.to_string()),
            }
        }
        ...
        fn tool_exception(label: &'static str, msg: &str) -> Self {
            Self {
                label,
                error: RustChainError::ToolException(msg.to_string()),
            }
        }
        ...
        fn http_error(label: &'static str, status: u16, body: &str) -> Self {
            Self {
                label,
                error: RustChainError::HttpError {
                    status,
                    body: body.to_string(),
                },
            }
    ...
    impl Runnable for Fails {
        ...
        async fn invoke(&self, _input: Value, _config: Option<&RunnableConfig>) -> Result<Value> {
            Err(match &self.error {
                RustChainError::Other(m) => RustChainError::Other(m.clone()),
                RustChainError::ToolException(m) => RustChainError::ToolException(m.clone()),
                RustChainError::HttpError { status, body } => RustChainError::HttpError {
                    status: *status,
                    body: body.clone(),
                },
                _ => RustChainError::Other("unexpected".into()),
            })
        }
    }
