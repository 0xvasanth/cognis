    ...
    match timeout_ms {
        Some(ms) => {
            ...
            match tokio::time::timeout(duration, runnable.invoke(input, config)).await {
                Ok(result) => result,
                Err(_) => Err(RustChainError::Other("batch item timed out".to_string())),
            }
        }
...
mod tests {
    ...
    fn fail_even() -> RunnableLambda {
        RunnableLambda::new("fail_even", |v: Value| async move {
            let n = v.as_i64().unwrap();
            if n % 2 == 0 {
                Err(RustChainError::Other(format!("even number: {n}")))
            } else {
                Ok(json!(n * 10))
            }
        })
        ...
        let runnable = Arc::new(RunnableLambda::new("retry_test", move |v: Value| {
            ...
            async move {
                ...
                if n == 0 {
                    ...
                    if count == 0 {
                        return Err(RustChainError::Other("transient error".into()));
                    }
                }
                Ok(json!(n * 10))
            }
