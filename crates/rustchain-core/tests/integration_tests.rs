...
fn error_types_comprehensive() {
    ...
    let err = RustChainError::NotImplemented("feature X".into());
...
async fn document_compressor_rag_workflow() {
    ...
    impl BaseDocumentCompressor for RelevanceCompressor {
        async fn compress_documents(
            &self,
            documents: &[rustchain_core::documents::Document],
            query: &str,
        ) -> rustchain_core::error::Result<Vec<rustchain_core::documents::Document>> {
            Ok(documents
                .iter()
                .filter(|d| {
                    query
