...
fn output_parser_error_display() {
    let err = RustChainError::OutputParserError {
...
fn context_overflow_display() {
    let err = RustChainError::ContextOverflow("too many tokens".into());
    assert_eq!(err.to_string(), "Context overflow: too many tokens");
}

#[test]
fn other_error_display() {
    let err = RustChainError::Other("something went wrong".into());
...
fn serde_json_error_converts() {
    ...
    let err: RustChainError = json_err.into();
    assert!(matches!(err, RustChainError::SerializationError(_)));
}

#[test]
fn result_type_alias_works() {
    let ok: rustchain_core::error::Result<i32> = Ok(42);
    assert_eq!(ok.unwrap(), 42);

    let err: rustchain_core::error::Result<i32> = Err(RustChainError::Other("fail".into()));
    assert!(err.is_err());
}

#[test]
fn tool_exception_display() {
    let err = RustChainError::ToolException("Calculator failed".into());
    assert_eq!(err.to_string(), "Tool exception: Calculator failed");
}

#[test]
fn tool_exception_matches() {
    let err = RustChainError::ToolException("test".into());
    assert!(matches!(err, RustChainError::ToolException(_)));
}
