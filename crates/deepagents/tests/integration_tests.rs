//! Integration tests for the deepagents crate.
//!
...
use deepagents::agent::{create_deep_agent, DeepAgentError};
use deepagents::backends::filesystem::FilesystemBackend;
use deepagents::backends::state::StateBackend;
use deepagents::backends::Backend;
use deepagents::config::DeepAgentConfig;
use deepagents::middleware::memory::MemoryMiddleware;
use deepagents::middleware::patch_tool_calls::PatchToolCallsMiddleware;
use deepagents::middleware::skills::{Skill, SkillsMiddleware};
use deepagents::middleware::summarization::SummarizationMiddleware;
...
async fn test_deep_agent_with_system_prompt() {
    ...
    // We verify the agent still runs successfully and returns the AI response.
    let last: Message = serde_json::from_value(messages.last().unwrap().clone()).unwrap();
...
async fn test_deep_agent_with_memory_middleware() {
    ...
    // The messages in state should include the injected memory context.
    // human + memory_system_msg + ai_response = 3
...
async fn test_patch_tool_calls_name_correction() {
    ...
    // Misspelled tool name: "calculater" should be corrected to "calculator".
    let mut state = json!({
        "messages": [
...
// ---------------------------------------------------------------------------
// 11. test_deep_agent_max_iterations (config field)
// ---------------------------------------------------------------------------

#[test]
fn test_deep_agent_max_iterations() {
    // Verify max_iterations is stored in the config (the graph itself does
    // not enforce it yet, but the config value should propagate correctly).
...
async fn test_memory_middleware_injects_context_into_state() {
    let mw = MemoryMiddleware::new(10);
    mw.remember("project", "rustchain").await;
...
fn test_filesystem_middleware_provides_tools() {
    use deepagents::middleware::filesystem::FilesystemMiddleware;
