        ...
        match self.router.pick(&ctx) {
            Some(idx) => {
                let sa = self.subagents.get(idx).ok_or_else(|| {
                    cognis2_core::CognisError::Configuration(format!(
...
mod tests {
    ...
    use cognis2_core::{Message, RunnableStream};
    use cognis2_llm::chat::{ChatOptions, ChatResponse, HealthStatus, StreamChunk};
