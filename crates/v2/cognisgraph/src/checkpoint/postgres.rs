    ...
    S: GraphState + Serialize + DeserializeOwned + Clone,
{
    async fn save(&self, run_id: Uuid, step: u64, state: &S) -> Result<()> {
        ...
        let json: serde_json::Value = if self.serializer.name() == "json" {
            ...
            let b64 = cognis2_core::base64_encode(&bytes);
            serde_json::json!({ "__cognis2_serialized__": b64 })
        };
        let stmt = format!(
        ...
        match row {
            ...
            Some(row) => {
                ...
                let state: S = if stored_serializer == "json" {
                    ...
                    let b64 = json["__cognis2_serialized__"].as_str().ok_or_else(|| {
                        CognisError::Serialization(
                            "non-json checkpoint missing __cognis2_serialized__ wrapper".into(),
                        )
                    })?;
                    let bytes = cognis2_core::base64_decode(b64)?;
