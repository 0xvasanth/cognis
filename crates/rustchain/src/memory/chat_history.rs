...
use rustchain_core::messages::{count_tokens_approximately, Message};
...
impl FileChatHistory {
    ...
    fn read_session_sync(&self, session_id: &str) -> Result<Vec<Message>> {
        ...
        let data = std::fs::read_to_string(&path).map_err(|e| {
            RustChainError::Other(format!("Failed to read session file {:?}: {}", path, e))
        })?;
    ...
    fn write_session_sync(&self, session_id: &str, messages: &[Message]) -> Result<()> {
        ...
        std::fs::write(&path, data).map_err(|e| {
            RustChainError::Other(format!("Failed to write session file {:?}: {}", path, e))
        })?;
        Ok(())
    }
}
    ...
    async fn list_sessions(&self) -> Result<Vec<String>> {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| {
                RustChainError::Other(format!("Failed to read directory entry: {}", e))
            })?;
            let path = entry.path();
...
///
/// Uses the approximate token counter from `rustchain_core`. Messages are
