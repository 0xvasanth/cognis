...
impl BaseTool for WebSearchTool {
    ...
    async fn _run(&self, input: ToolInput) -> Result<ToolOutput> {
        ...
        let resp = req.send().await.map_err(|e| {
            RustChainError::ToolException(format!("Web search request failed: {e}"))
        })?;
        ...
        let body = resp.text().await.map_err(|e| {
            RustChainError::ToolException(format!("Failed to read search response: {e}"))
        })?;
...
impl BaseTool for DuckDuckGoSearchTool {
    ...
    async fn _run(&self, input: ToolInput) -> Result<ToolOutput> {
        ...
        let resp = self
            ...
            .await
            .map_err(|e| {
                RustChainError::ToolException(format!("DuckDuckGo request failed: {e}"))
            })?;
        ...
        let ddg: DdgResponse = resp.json().await.map_err(|e| {
            RustChainError::ToolException(format!("Failed to parse DuckDuckGo response: {e}"))
        })?;
