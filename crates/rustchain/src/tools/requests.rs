...
impl RequestsTool {
    ...
    fn validate_domain(&self, url: &str) -> Result<()> {
        if let Some(ref allowed) = self.config.allowed_domains {
            ...
            {
                return Err(RustChainError::ToolValidationError(format!(
        ...
        match input {
            ...
            ToolInput::Structured(map) => {
                let value = serde_json::to_value(map).map_err(|e| {
                    RustChainError::ToolValidationError(format!("Failed to serialize input: {e}"))
                })?;
                serde_json::from_value::<RequestInput>(value).map_err(|e| {
                    RustChainError::ToolValidationError(format!("Invalid request input: {e}"))
                })
            }
            ToolInput::ToolCall(tc) => {
                let value = serde_json::to_value(&tc.args).map_err(|e| {
                    RustChainError::ToolValidationError(format!("Failed to serialize args: {e}"))
                })?;
                serde_json::from_value::<RequestInput>(value).map_err(|e| {
                    RustChainError::ToolValidationError(format!("Invalid request input: {e}"))
                })
...
impl BaseTool for RequestsTool {
    ...
    async fn _run(&self, input: ToolInput) -> Result<ToolOutput> {
        ...
        let method = if let Some(ref m) = req.method {
            m.parse::<HttpMethod>()
                .map_err(RustChainError::ToolValidationError)?
        } else {
            self.default_method
        };
...
mod tests {
    ...
    async fn test_requests_tool_domain_allowlist_reject() {
        ...
        match result.unwrap_err() {
            RustChainError::ToolValidationError(msg) => {
