...
impl FallbackChain {
    ...
    pub async fn generate(
        ...
        let mut last_err = RustChainError::Other("FallbackChain has no models".into());
...
impl ModelRouter {
        ...
        let chosen = match &self.strategy {
            ...
            RoutingStrategy::Custom(selector) => {
                ...
                let local_idx = selector.select(&profiles, context).ok_or_else(|| {
                    RustChainError::Other("Custom selector returned no candidate".into())
                })?;
    ...
    pub async fn route_with_fallback(
        ...
        let mut last_err = RustChainError::Other("No candidates".into());
    ...
    impl BaseChatModel for StubModel {
        async fn _generate(
            ...
            if self.should_fail.load(Ordering::Relaxed) {
                return Err(RustChainError::Other(format!("{} failed", self.name)));
            }
