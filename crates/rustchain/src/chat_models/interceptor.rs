...
use rustchain_core::messages::{Message, SystemMessage};
...
impl ContentFilter {
    ...
    pub fn new(pattern: &str, replacement: impl Into<String>) -> Result<Self> {
        let re = Regex::new(pattern)
            .map_err(|e| RustChainError::Other(format!("Invalid regex: {e}")))?;
        Ok(Self {
            pattern: re,
...
impl InterceptedChatModelBuilder {
    ...
    pub fn build(self) -> Result<InterceptedChatModel> {
        let inner = self.inner.ok_or_else(|| {
            RustChainError::Other("InterceptedChatModelBuilder: inner model is required".into())
        })?;
...
mod tests {
    ...
    async fn test_response_validator_min_length_pass() {
        ...
        let mut result = ChatResult {
            generations: vec![rustchain_core::outputs::ChatGeneration::new(
                rustchain_core::messages::AIMessage::new("hello"),
            )],
            llm_output: None,
        };
    ...
    async fn test_response_validator_min_length_fail() {
        ...
        let mut result = ChatResult {
            generations: vec![rustchain_core::outputs::ChatGeneration::new(
                rustchain_core::messages::AIMessage::new("hi"),
            )],
            llm_output: None,
        };
    ...
    async fn test_response_transformer_strips_prefix() {
        ...
        let mut result = ChatResult {
            generations: vec![rustchain_core::outputs::ChatGeneration::new(
                rustchain_core::messages::AIMessage::new("AI: hello there"),
            )],
            llm_output: None,
        };
    ...
    async fn test_token_counter_counts_tokens() {
        ...
        let mut result = ChatResult {
            generations: vec![rustchain_core::outputs::ChatGeneration::new(
                rustchain_core::messages::AIMessage::new("one two three four"),
            )],
            llm_output: None,
        };
    ...
    async fn test_response_logger_captures_response() {
        ...
        let mut result = ChatResult {
            generations: vec![rustchain_core::outputs::ChatGeneration::new(
                rustchain_core::messages::AIMessage::new("captured"),
            )],
            llm_output: None,
        };
