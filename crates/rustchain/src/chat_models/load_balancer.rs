...
impl ModelHealth {
    ...
    fn record_failure(&self, error: &RustChainError, unhealthy_threshold: usize) {
        self.total_requests.fetch_add(1, Ordering::Relaxed);
...
///
/// ```rust,ignore
/// use rustchain::chat_models::load_balancer::{LoadBalancedChatModel, LoadBalancingStrategy};
///
...
mod tests {
    use super::*;
    use rustchain_core::messages::{AIMessage, AIMessageChunk, HumanMessage};
    use rustchain_core::outputs::{ChatGeneration, ChatGenerationChunk};
