...
///
/// ```rust,ignore
/// use rustchain::chat_models::graceful::GracefulChatModel;
///
...
impl GracefulChatModel {
    ...
    where
        F: Fn(&RustChainError) + Send + Sync + 'static,
    {
    ...
    async fn _stream(&self, messages: &[Message], stop: Option<&[String]>) -> Result<ChatStream> {
        match self.inner._stream(messages, stop).await {
            ...
            Err(e) => {
                ...
                use futures::stream;
                use rustchain_core::messages::AIMessageChunk;
                use rustchain_core::outputs::ChatGenerationChunk;
...
mod tests {
    use super::*;
    use rustchain_core::messages::HumanMessage;
