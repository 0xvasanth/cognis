...
mod tests {
    ...
    fn test_retry_on_any_error() {
        ...
        assert!(cond.matches(&RustChainError::Other("test".into())));
    }

    #[test]
    fn test_retry_on_error_contains() {
        let cond = RetryOn::ErrorContains(vec!["timeout".into()]);
        assert!(cond.matches(&RustChainError::Other("connection timeout".into())));
        assert!(!cond.matches(&RustChainError::Other("bad input".into())));
    }

    #[test]
    fn test_retry_on_error_types() {
        let cond = RetryOn::ErrorTypes(vec!["ToolException".into()]);
        assert!(cond.matches(&RustChainError::ToolException("fail".into())));
        assert!(!cond.matches(&RustChainError::Other("fail".into())));
    }
