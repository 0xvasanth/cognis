...
use langgraph::graph::state::{AsyncNodeAction, CompiledStateGraph, StateGraph};
use langgraph::types::{InterruptType, InvokeResult, Send as GraphSend, StreamMode};
...
mod tool_agent_helpers {
    ...
    pub fn make_tool_agent_test() -> CompiledStateGraph {
        ...
        langgraph::prebuilt::create_tool_agent(model, vec![tool], None).unwrap()
    }
}
...
async fn test_tool_agent_end_to_end() {
    ...
    let last: rustchain_core::messages::Message =
