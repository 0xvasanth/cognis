...
## What about LangSmith and OpenTelemetry?

Both are on the roadmap. Until they ship:

- For **OTel**: implement `TraceExporter` (it's a thin async trait) and forward to your collector.
- For **LangSmith**: implement `TraceExporter` against the LangSmith ingest API.

The `TraceExporter` interface is small, and the batching infrastructure is already there — you only write the format conversion.
