...
`cognis-trace` translates Cognis runtime events into spans, generations, and scores, then ships them to one or more exporters. Langfuse is the supported production backend; stdout and mock exporters ship for local use; LangSmith and OpenTelemetry are on the roadmap.
...
Implement for LangSmith, OTel, your own backend.
