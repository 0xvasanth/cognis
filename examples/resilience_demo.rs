...
fn main() {
    ...
    for attempt in 0..4 {
        println!("  attempt {}: delay = {}ms", attempt, fixed.delay_for_attempt(attempt));
    }
    ...
    for attempt in 0..8 {
        println!("  attempt {}: delay = {}ms", attempt, exponential.delay_for_attempt(attempt));
    }
    ...
    println!("\nStrategy: {}", linear);
    for attempt in 0..12 {
        println!("  attempt {}: delay = {}ms", attempt, linear.delay_for_attempt(attempt));
    }

    // None strategy
    let none = RetryStrategy::None;
    println!("\nStrategy: {} -> delay = {}ms", none, none.delay_for_attempt(0));
    ...
    println!("\nConfig JSON: {}", serde_json::to_string_pretty(&config.to_json()).unwrap());
    ...
    println!("Initial state: {} (available={})", cb.state(), cb.is_available());

    // Record successes
    cb.record_success();
    cb.record_success();
    println!("After 2 successes: state={}, failures={}", cb.state(), cb.failure_count());

    // Record failures up to threshold
    cb.record_failure();
    println!("After 1 failure: state={}, failures={}", cb.state(), cb.failure_count());
    cb.record_failure();
    println!("After 2 failures: state={}, failures={}", cb.state(), cb.failure_count());
    cb.record_failure();
    println!("After 3 failures (threshold): state={}, available={}", cb.state(), cb.is_available());
    ...
    cb.record_success();
    println!("After success in HalfOpen: state={}, failures={}", cb.state(), cb.failure_count());
    ...
    cb.reset();
    println!("After reset(): state={}, available={}", cb.state(), cb.is_available());

    println!("Breaker JSON: {}", serde_json::to_string_pretty(&cb.to_json()).unwrap());
    ...
    println!("Bulkhead: max={}, available={}, active={}", bh.max_concurrent(), bh.available(), bh.active());

    // Acquire permits
    let p1 = bh.try_acquire();
    println!("Acquired permit 1: success={}, available={}, active={}", p1.is_some(), bh.available(), bh.active());

    let p2 = bh.try_acquire();
    println!("Acquired permit 2: success={}, available={}, active={}", p2.is_some(), bh.available(), bh.active());

    let p3 = bh.try_acquire();
    println!("Acquired permit 3: success={}, available={}, active={}", p3.is_some(), bh.available(), bh.active());

    // Fourth should fail
    let p4 = bh.try_acquire();
    println!("Acquired permit 4: success={} (bulkhead full)", p4.is_some());

    // Drop a permit to free a slot
    drop(p1);
    println!("After dropping permit 1: available={}, active={}", bh.available(), bh.active());

    let p5 = bh.try_acquire();
    println!("Acquired permit 5 (after release): success={}", p5.is_some());

    // Drop all remaining permits
    drop(p2);
    drop(p3);
    drop(p5);
    println!("After dropping all permits: available={}, active={}", bh.available(), bh.active());

    println!("Bulkhead JSON: {}", serde_json::to_string_pretty(&bh.to_json()).unwrap());
    ...
    println!("  available={}, next={:?}", chain.available_count(), chain.next_fallback());
    ...
    println!("Chain JSON: {}", serde_json::to_string_pretty(&chain.to_json()).unwrap());
    ...
    println!("Policy '{}': can_execute={}", policy.name, policy.can_execute());
    ...
    if let Some(ref retry) = policy.retry_config {
        ...
        println!("  should_retry(0, 'timeout')={}", retry.should_retry(0, "timeout"));
        println!("  should_retry(0, 'auth_error')={}", retry.should_retry(0, "auth_error"));
    }

    println!("\nPolicy JSON: {}", serde_json::to_string_pretty(&policy.to_json()).unwrap());
    ...
    println!("\nMetrics JSON: {}", serde_json::to_string_pretty(&metrics.to_json()).unwrap());
